...
public:
  ...
  // Allows to process the ssame graphs in the experiments
  ...
  // Generates a graph and calls MakeConnected()
  ...
  // Checks if the algorithm may be used
  bool IsConnected() const;
  bool IsValidForDijkstra() const;

  // Kind of the same due to similar realisation of heaps
  std::vector<double> Dijkstra(int start_vertex) const; // Binary heap
...
AdjacencyList::AdjacencyList(const Graph& matrix_graph) 
{
  ...
  // Initializes the elements in the list
  for (int i = 0; i < n; ++i) 
  {
...
void AdjacencyList::AddEdge(int from, int to, double weight) 
{
  ...
  // In case it's necessary to add a vertex
...
// Connects the components with a single edhe
